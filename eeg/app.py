...
Combines calibration, training, real-time prediction, and Mindful Ascent balloon game
"""
...
class AppState:
    def __init__(self):
        ...
        self.buffer = deque(maxlen=int(FS * 10))
        self.recording_data = []
        self.model = None
        self.scaler = None
...
def process_calibration_file(filename, label):
    ...
    filtered_df = pd.DataFrame({"Time(s)": time_vals, "Filtered_Voltage": filtered})
    filtered_df.to_csv(os.path.join(PIPELINE_DIR, f"{label}_filtered.csv"), index=False)
    ...
    feature_df = pd.DataFrame(features, columns=["Window", "Alpha", "Beta", "AlphaBetaRatio"])
    feature_df.to_csv(os.path.join(PIPELINE_DIR, f"{label}_features.csv"), index=False)
                        ...
                        if raw.isdigit():
                            ...
                            if state.is_recording:
                                ...
                                socketio.emit('waveform_data', {
                                    'time': t, 'voltage': voltage, 'mode': 'recording'
                                })
                                ...
                                if samples_received % 10 == 0:
                                    socketio.emit('waveform_data', {
                                        ...
                                        'voltage': voltage, 'mode': 'prediction'
                                    })
        ...
        if samples_since_last >= step_size:
            ...
            try:
                ...
                if beta > 0:
                    ...
                    print(f"[Prediction] {prediction:10s} | α={alpha:.2e}  β={beta:.2e}  ratio={ratio:.3f}  conf={confidence:.1%}")
...
def stop_recording():
    ...
    df.to_csv(os.path.join(CALIBRATION_DIR, filename), index=False)

    try:
        num_windows = process_calibration_file(filename, state.recording_mode)
        return jsonify({
            'success': True,
            'message': f'Saved {len(state.recording_data)} samples, extracted {num_windows} feature windows',
            'filename': filename
        })
    except Exception as e:
...
def start_prediction():
    ...
    try:
        ...
        print("\n" + "="*60)
        print("PREDICTION MODE STARTED — Balloon game active")
        print(f"  Window: {WINDOW_SEC}s | Step: {STEP_SEC}s | Rate: {FS}Hz")
        print("="*60 + "\n")

        return jsonify({'success': True, 'message': 'Prediction started — switch to 🎈 Mindful Ascent to play!'})
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("  EEG CALMNESS MONITOR + MINDFUL ASCENT")
    print("="*60)
    print("\n  Open browser → http://localhost:5000")
    print("\n  Workflow:")
    print("  1. ⚡ EEG Monitor tab → Connect → Calibrate → Train")
    print("  2. ⚡ EEG Monitor tab → Start Prediction")
    print("  3. 🎈 Mindful Ascent tab → Start Session → Relax & fly!")
